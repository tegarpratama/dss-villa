...
@section('content')
    <div class="card p-4">
        <div class="row">
            <div class="col">
                <h4>Ubah Data Penginapan</h4>
            </div>
        </div>
        <div class="row mt-4">
            <div class="col">
                <form action="{{ route('penginapan.update', $villa->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label>Nama</label>
                        <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name') ? old('name') : $villa->name }}">
                        @error('name')
